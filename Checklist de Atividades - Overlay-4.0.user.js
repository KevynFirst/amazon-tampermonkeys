// ==UserScript==
// @name         Checklist de Atividades - Overlay
...
(function () {
    ...
    function createOnbModule() {
        ...
        function injectOverlay(r) {
            ...
            head.innerHTML = '<div style="font-size:14px;font-weight:700;">' + (dia ? '☀️' : '🌙') + ' Alertas de Onboarding <span style="font-size:11px;font-weight:600;opacity:.85;">(' + (dia ? 'Dia' : 'Noite') + ')</span></div>';
            ...
            btnFlt.onclick = () => { setLimitByHours(!limitByHours); ov.remove(); injectOverlay(r); };
            const btnDet = document.createElement('button'); btnDet.innerHTML = '🔎 Mais detalhes'; btnDet.style.cssText = 'background:' + C.accent + ';color:#232F3E;border:none;border-radius:6px;padding:5px 10px;cursor:pointer;font-weight:700;font-size:12px;'; btnDet.onclick = () => { ov.remove(); showDashboard(r); };
            ...
            headBtns.appendChild(btnFlt); headBtns.appendChild(btnDet); headBtns.appendChild(x); head.appendChild(headBtns);
            ...
            tabHora.innerHTML = (limitByHours ? '⏰ ' : '📋 ') + listTitle() + ' (' + exceeding.length + ')'; tabLog.innerHTML = '🔁 Precisa logar (' + faltantes.length + ')';
            ...
            function setActive(which) { tabHora.style.cssText = tabBase + (which === 'hora' ? 'color:' + C.red + ';border-bottom:3px solid ' + C.red + ';' : 'color:' + C.grey + ';border-bottom:3px solid transparent;'); tabLog.style.cssText = tabBase + (which === 'log' ? 'color:' + C.amber + ';border-bottom:3px solid ' + C.amber + ';' : 'color:' + C.grey + ';border-bottom:3px solid transparent;'); if (which === 'hora') renderHora(); else renderLog(); }
