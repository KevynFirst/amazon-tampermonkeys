...
// @updateURL    https://raw.githubusercontent.com/KevynFirst/amazon-tampermonkeys/main/Minichecklist%20Mecanismos-5.0.user.js
// @downloadURL  https://raw.githubusercontent.com/KevynFirst/amazon-tampermonkeys/main/Minichecklist%20Mecanismos-5.0.user.js
// ==/UserScript==
//
...
//
// v5.1 — Edição dos itens PADRÃO (igual ao Minichecklist Learning):
//        • Aba "➕ Adicionar" virou "✏️ Editar": lista TODAS as tarefas do cargo.
//        • Tarefa padrão pode ser renomeada, ter o link/alerta trocados, receber
//          recorrência (todo dia / dias da semana / única), ser marcada N/A, ocultada
//          e reordenada (arrastar). Nada é apagado do código: as edições viram
//          "overrides" salvos em chkmec_overrides, com "↺ Restaurar padrão" p/ desfazer.
//        • Itens pessoais agora também são editáveis (texto/link/alerta/recorrência).
//        • ID das tarefas fixas passou a usar o índice ORIGINAL do MASTER (id estável
//          ao ocultar itens). Isso zera os checks do dia corrente na 1ª carga.
//
(function () {
    'use strict';
    ...
    // Itens do cargo COM o índice ORIGINAL do MASTER — o id da tarefa é '<cargo>_<idxOriginal>',
    // então ocultar/reordenar itens não faz as edições "escorregarem" para o vizinho.
    function masterEntries(sel) {
        const out = [];
        MASTER.forEach((a, idx) => { if (!a.roles || a.roles.indexOf(sel) >= 0) out.push({ a: a, idx: idx }); });
        return out;
    }
    ...
    // Edições feitas nas tarefas PADRÃO (aba "✏️ Editar"). Persistem entre dias/turnos.
    // { hidden:{id:true}, labels:{id:'texto'}, urls:{id:'url'}, alerts:{id:'HH:MM'},
    //   order:{cargo:[ids]}, repeat:{id:'daily|weekly|once'}, rdays:{id:[0..6]} }
    const K_OVERRIDES = 'chkmec_overrides';
    ...
    let dragId = null;   // id da linha sendo arrastada na aba "✏️ Editar"

    // ── Edições das tarefas PADRÃO (renomear / relinkar / ocultar / reordenar) ──
    // Os defaults do MASTER nunca são alterados: guardamos só os "overrides" por id
    // e o merge é feito na renderização (computeState).
    function emptyOverrides() { return { hidden: {}, labels: {}, urls: {}, alerts: {}, order: {}, repeat: {}, rdays: {} }; }
    function getOverrides() {
        try {
            const v = JSON.parse(store.get(K_OVERRIDES, 'null'));
            if (v && typeof v === 'object') return { hidden: v.hidden || {}, labels: v.labels || {}, urls: v.urls || {}, alerts: v.alerts || {}, order: v.order || {}, repeat: v.repeat || {}, rdays: v.rdays || {} };
        } catch (e) {}
    ...
    // Ao virar o dia: tarefas padrão marcadas como ÚNICAS e já concluídas são ocultadas.
    ...
    function resetOverrides() { store.set(K_OVERRIDES, JSON.stringify(emptyOverrides())); }
    // Ordem manual das tarefas (por cargo) — usada no arrastar-para-reordenar.
    ...
    function applyOrder(items, sel) {
        ...
        return items.map((it, idx) => ({ it: it, idx: idx })).sort((a, b) => {
            const pa = pos[a.it.id] == null ? 1e9 + a.idx : pos[a.it.id];
            const pb = pos[b.it.id] == null ? 1e9 + b.idx : pos[b.it.id];
            return pa - pb;
        }).map(x => x.it);
    }
    ...
    function ensureCycle() {
        ...
        let c = getCycle();
        if (!c || c.opDate !== opd) {
            // Virou o dia: itens marcados como "única" e já concluídos saem da lista.
            if (c && c.done) { purgeDoneOnceCustom(c.done); purgeDoneOnceFixed(c.done); }
            c = { opDate: opd, shift: shiftFromTime(now), shiftManual: false, selection: null, done: {}, na: {} };
            setCycle(c);
            warnedIds = {}; beepedIds = {};
        }
        if (!c.na) c.na = {};   // compatibilidade com ciclos salvos antes da v5.1
        return c;
    }
    ...
    // N/A ("não aplicável"): satisfaz a tarefa sem contar como feita (não alerta e conta no %).
    function toggleNa(id) { const c = ensureCycle(); if (!c.na) c.na = {}; if (c.na[id]) delete c.na[id]; else { c.na[id] = true; if (c.done) delete c.done[id]; } setCycle(c); }
    function reAsk() { const c = ensureCycle(); c.selection = null; c.manager = null; c.managerName = null; c.managerLogin = null; c.done = {}; c.na = {}; setCycle(c); warnedIds = {}; beepedIds = {}; }
    ...
    function computeState() {
        ...
        const shift = currentShift(c), done = c.done || {}, naMap = c.na || {}, snoozeMap = c.snooze || {}, t = nowMs();
        const ov = getOverrides();
        const items = [];
        // Tarefas padrão do cargo, com as edições da aba "✏️ Editar" aplicadas por cima.
        masterEntries(c.selection).forEach(e => {
            const a = e.a, id = c.selection + '_' + e.idx;
            if (a.dayOnly && shift !== 'day') return;
            if (ov.hidden[id]) return;                       // tarefa ocultada na aba Editar
            ...
            items.push({
                id: id, label: label, desc: desc, url: url, apolloMetric: a.apolloMetric || null,
                periodKey: periodKey, periodId: periodId, alert: hhmm || null, ts: ts,
                repeat: frep, days: fdays,
                done: isDone, na: isNa, satisfied: satisfied, secsLeft: secsLeft,
        ...
        // Itens pessoais adicionados pelo usuário (aba "✏️ Editar") — done por dia como os demais.
        ...
        getCustom().forEach(ci => {
            ...
            items.push({
                ...
                alert: hhmm, ts: ts, done: isDone, na: isNa, satisfied: satisfied, secsLeft: secsLeft,
                snoozed: snoozed, snoozeLeft: snoozed ? Math.round((snoozeUntil - t) / 1000) : 0,
                overdue: !!ts && !satisfied && !snoozed && t >= ts,
                warning: !!ts && !satisfied && !snoozed && secsLeft > 0 && secsLeft <= WARN_SEC,
        ...
        const ordered = applyOrder(items, c.selection);   // ordem manual (arrastar) por cargo
        ...
        return { needSetup: false, selection: c.selection, manager: c.manager || null, shift, items: ordered, total, doneCount, pct, overdue, warning };
    }
    ...
    function buildUI() {
        ...
        // Abas: Tarefas · ✏️ Editar (edita as tarefas padrão + itens pessoais).
    ...
    function toggleHelp() {
        ...
        bd.appendChild(el('p', 'font-size:12.5px;line-height:1.6;color:#37475A;margin:0 0 10px;',
            'Na aba “✏️ Editar” você personaliza a lista: renomeie uma tarefa padrão, troque o link ou o horário do alerta, defina a recorrência (todo dia / dias da semana / única), marque N/A, exclua ou arraste pela alça ⠿ para reordenar. Também dá para criar itens pessoais. Nada é perdido: “↺ Restaurar tarefas padrão” devolve as tarefas originais.'));
    ...
    function buildRow(i) {
        ...
        if (i.desc) mid.appendChild(el('div', 'font-size:11px;line-height:1.3;margin-top:1px;color:' + (i.satisfied ? '#9AA7B4' : '#5B6B7B') + ';', i.desc));
        let stTxt, stColor;
    ...
    // ── Abas do checklist (Tarefas · ✏️ Editar) ─────────────────────────
    ...
    // Controle de repetição: Todo dia · Dias da semana · Única.
    ...
    // Monta a aba "✏️ Editar": formulário para adicionar + TODAS as tarefas do cargo
    // (padrão e pessoais) com editar / N-A / excluir / arrastar para reordenar.
    let editingKey = null;   // id da tarefa em edição (abre o formulário inline)
    function buildEditView() {
        ...
        naLbl.appendChild(naChk); naLbl.appendChild(el('span', '', '🚫 marcar como N/A'));
        ...
        // ── Todas as tarefas do cargo atual (editar / N/A / excluir / reordenar) ──
        const s = computeState();
        if (s.needSetup) {
            listEl.appendChild(el('div', 'padding:14px 10px;text-align:center;color:#5B6B7B;font-size:12.5px;line-height:1.5;',
                'Escolha o cargo do turno para ver e editar as tarefas.'));
            return;
        }
        listEl.appendChild(el('div', 'font-size:11px;font-weight:800;color:#5B6B7B;margin:4px 2px 8px;letter-spacing:.04em;', 'TAREFAS DO CARGO'));
        const c2 = ensureCycle(); const sel = c2.selection; const ov = getOverrides();
        let rows = [];
        // Tarefas padrão: TODAS do cargo (mesmo as que não valem hoje), já com os overrides.
        masterEntries(sel).forEach(e => {
            const a = e.a, id = sel + '_' + e.idx;
            if (ov.hidden[id]) return;
            const hhmm = ov.alerts.hasOwnProperty(id) ? (ov.alerts[id] || null) : ((currentShift(c2) === 'night') ? (a.night || null) : (a.day || null));
            rows.push({
                id: id, label: ov.labels[id] || a.t,
                url: ov.urls.hasOwnProperty(id) ? (ov.urls[id] || null) : (a.url || null),
                alert: hhmm, na: !!(c2.na || {})[id],
                repeat: ov.repeat[id] || 'daily', days: ov.rdays[id] || [],
                periodKey: a.periodKey || null, periodType: a.periodType || null,
            });
        });
        // Itens pessoais (todos, mesmo os que não valem hoje — para poder editar).
        getCustom().forEach(ci => {
            const id = 'cust_' + ci.id;
            rows.push({ id: id, label: ci.t, url: ci.url || null, alert: ci.alert || null, na: !!(c2.na || {})[id], custom: true, customId: ci.id, repeat: ci.repeat || 'daily', days: ci.days || [] });
        });
        ...
        // Restaurar tarefas padrão (desfaz exclusões, renomeações, links, alertas e ordem).
        const reset = el('button', 'width:100%;margin-top:6px;background:#EAEDF0;border:1px solid #CBD3DB;color:#5B6B7B;'
            + 'border-radius:8px;padding:8px 12px;cursor:pointer;font-size:12px;font-weight:700;' + AMZ, '↺ Restaurar tarefas padrão');
        reset.title = 'Desfaz exclusões, renomeações, links, alertas e a ordem das tarefas padrão (não afeta itens pessoais)';
        reset.addEventListener('click', (e) => { e.stopPropagation(); resetOverrides(); editingKey = null; lastListSig = ''; render(); });
        listEl.appendChild(reset);
    }

    // Linha compacta: alça p/ arrastar · texto (clica p/ editar) · N/A rápido · ✏️.
    function buildEditRowCompact(i, orderIds, selection) {
        ...
        row.appendChild(pen);
        // Arrastar para reordenar (a ordem é salva por cargo).
        row.draggable = true;
        ...
        row.addEventListener('drop', (e) => {
            e.preventDefault(); row.style.borderColor = '#E3E7EC';
            const src = dragId || (e.dataTransfer && e.dataTransfer.getData('text/plain'));
            if (src && src !== i.id) { const ids = orderIds.slice(); reorderArr(ids, src, i.id); setOrder(selection, ids); lastListSig = ''; buildEditView(); }
        });
        return row;
    }

    // Formulário de edição inline (texto + link + alerta + recorrência + N/A + salvar/excluir).
    function buildEditForm(i) {
        ...
        const txtInp = el('input', fieldCss); txtInp.type = 'text'; txtInp.value = i.label; txtInp.placeholder = 'Texto da tarefa…';
        ...
        // Repetição — para qualquer tarefa (padrão ou pessoal).
        const repCtl = buildRepeatControl(wrap, { repeat: i.repeat, days: i.days });

        // N/A — mesmo estilo da linha de Alerta. Aplicado ao Salvar.
        const naRow = el('div', 'display:flex;align-items:center;gap:8px;margin-top:9px;');
        const naLbl = el('label', 'display:flex;align-items:center;gap:6px;cursor:pointer;font-size:12.5px;font-weight:700;color:#232F3E;user-select:none;' + AMZ);
        ...
        // Aviso: tarefas com check semanal/quinzenal (FSI, Area Organization).
        if (i.periodKey) {
            wrap.appendChild(el('div', 'margin-top:8px;font-size:10.5px;font-weight:700;color:#7C8B99;line-height:1.4;',
                i.periodType === 'biweek' ? 'ℹ️ O check desta tarefa vale por quinzena.' : 'ℹ️ O check desta tarefa vale pela semana (Dom–Sáb).'));
        }
        ...
        del.title = i.custom ? 'Remover item pessoal' : 'Excluir esta tarefa (dá para desfazer em “Restaurar tarefas padrão”)';
    ...
    function render() {
        ...
        if (menuOpen) {
            ...
            if (chkTab === 'edit') {
                // Aba "✏️ Editar": enquanto um item está em edição NÃO reconstrói
                // (preserva o formulário aberto e o que já foi digitado).
                if (!editingKey) {
                    const sig = 'edit|' + s.items.map(i => i.id + '~' + i.label + '~' + (i.na ? 'N' : '') + (i.done ? 'D' : '')).join('|')
                        + '|ov=' + store.get(K_OVERRIDES, '')
                        + '|cust=' + getCustom().map(c => c.id + ':' + c.t + ':' + (c.repeat || '') + ':' + ((c.days || []).join('')) + ':' + (c.alert || '') + ':' + (c.url || '')).join('|');
