...
// @updateURL    https://raw.githubusercontent.com/KevynFirst/amazon-tampermonkeys/main/Minichecklist%20Learning-5.0.user.js
// @downloadURL  https://raw.githubusercontent.com/KevynFirst/amazon-tampermonkeys/main/Minichecklist%20Learning-5.0.user.js
// ==/UserScript==
//
...
(function () {
    ...
    const K_HIDDEN = 'chkatv_hidden'; // '1' = círculo (FAB) oculto via Ctrl+Alt+H
    ...
    // Círculo oculto pelo atalho Ctrl+Alt+H (persiste). Não afeta alertas nem o Onboarding Hours.
    let fabHidden = (store.get(K_HIDDEN, '') === '1');
    function toggleFabHidden() {
        fabHidden = !fabHidden;
        store.set(K_HIDDEN, fabHidden ? '1' : '');
        if (fabHidden) { menuOpen = false; setMenuVisible(false); if (radialEl) hideRadial(); }
        render();
    }
    ...
    function applyGroupVisibility() {
        const hide = overlayOpen() || fabHidden;   // Ctrl+Alt+H também oculta o círculo
    ...
    function updateRing() {
        ...
        if (!onbAvail || overlayOpen() || fabHidden || erradoCount <= 0) {
    ...
    function render() {
        ...
        // A lista NÃO abre sozinha a cada hora cheia (atrapalhava). Só abre no clique do
        // círculo. Os alertas por horário continuam funcionando normalmente.
    ...
    function createOnbModule() {
        ...
        // (REPORT_LINK removido: era uma URL fixa sem a janela do turno e
        //  levava a uma página diferente da que gerou os números na tela.
        //  Todos os links agora saem de reportUrl()/reportUrlFor().)
        ...
        const PROCESSES = [
            ...
            { key: 'log', name: '🔁 Precisa logar', virtual: true },    // faltando em calm code (Dia 1/2)
        ...
        // Janela SEMPRE automática pelo turno atual de hoje (sem filtro selecionável).
        ...
        function reportUrl(processId) {
            // SEM filtro selecionável: a janela é SEMPRE o intraday do turno de hoje.
            const p = autoShiftParams();
        ...
        // ── Links de origem (mesmo padrão do FCLM Hours) ──────────────────
        // O 🔗 abre o functionRollup do processo daquele item JÁ com a janela
        // do turno atual aplicada (autoShiftParams) — ou seja, a página
        // exatamente como foi filtrada para gerar o número exibido.
        const TIP_SRC = 'Abrir no FCLM o relatório de onde veio esta informação';
        function reportUrlFor(t) { return reportUrl(procOf(t).processId); }
        function linkTag(url, tip) {
            return '<a href="' + esc(url) + '" target="_blank" rel="noopener" title="'
                + esc(tip) + '" style="text-decoration:none;">🔗</a> ';
        }
        // t = objeto de treinamento/entrada (usa .fnId/.title) ou o próprio título/calm code
        function srcLink(t, extra) {
            return linkTag(reportUrlFor(t), TIP_SRC + (extra ? ' — ' + extra : ''));
        }
        // Link pelo NOME do processo (usado no agrupamento de Ajuste de Badge)
        function srcLinkProcName(procName) {
            const p = PROCESSES.find(x => x.name === procName && x.processId);
            return linkTag(reportUrl(p ? p.processId : ONB_PROCESS),
                           TIP_SRC + (procName ? ' — ' + procName : ''));
        }
        ...
        function showPeopleModal(training) {
            ...
            const body = document.createElement('div'); body.style.cssText = 'flex:1;overflow-y:auto;padding:18px 20px;background:' + C.bodyBg + ';';
            // Barra com o link para o relatório de origem (janela do turno já aplicada)
            let html = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;gap:10px;flex-wrap:wrap;">'
                + '<span style="font-size:12px;color:' + C.grey + ';font-weight:700;">' + esc(procOf(training).name) + ' · limite ' + lim + 'h</span>'
                + '<button id="onb-openfclm" style="background:#fff;color:' + C.dark + ';border:1px solid #CDD4DA;border-radius:8px;padding:8px 16px;cursor:pointer;font-weight:800;font-size:13px;">🔗 Abrir no FCLM</button>'
                + '</div>';
            ...
            // Abre o relatório desta função com a janela do turno já aplicada
            const ofc = document.getElementById('onb-openfclm');
            if (ofc) ofc.onclick = () => { const u = reportUrlFor(training); try { window.open(u, '_blank', 'noopener'); } catch (e) { location.href = u; } };
        }
        function buildDashHTML(fr) {
            ...
            fr.trainings.forEach((t, i) => { html += '<div class="onb-train-card" data-idx="' + i + '" style="background:#fff;border:1px solid ' + C.border + ';border-left:4px solid ' + C.accent + ';border-radius:10px;padding:14px 16px;cursor:pointer;box-shadow:0 2px 8px rgba(35,47,62,0.06);transition:all .15s ease;"><div style="font-size:15px;font-weight:700;color:' + C.dark + ';">' + srcLink(t, t.title) + esc(t.title) + '</div><div style="font-size:26px;font-weight:800;color:' + C.blue + ';margin-top:4px;">' + t.people.length + ' <span style="font-size:13px;color:' + C.grey + ';font-weight:600;">associado(s)</span></div></div>'; });
            html += '</div>';
        ...
        function buildLogHTML(fr) {
            ...
            if (codes.length) {
                codes.forEach(code => {
                    html += '<div style="margin:10px 0 4px;font-size:13px;font-weight:800;color:#fff;background:' + C.dark + ';padding:6px 12px;border-radius:6px;border-left:4px solid ' + C.accent + ';">' + srcLink(code, code) + '🎓 ' + esc(code) + ' <span style="font-weight:600;color:' + C.gold + ';">(' + byCode[code].length + ')</span></div>';
                    byCode[code].forEach(p => { html += '<div style="font-size:15px;color:' + C.dark + ';padding:3px 0 3px 10px;">' + nameLink(p.name, p.link) + '</div>'; });
                });
        ...
        function buildBadgeHTML(fr) {
            ...
            if (list.length) {
                ...
                Object.keys(by).sort((a, b) => order.indexOf(a) - order.indexOf(b)).forEach(pn => {
                    html += '<div style="margin:10px 0 4px;font-size:13px;font-weight:800;color:#fff;background:' + C.dark + ';padding:6px 12px;border-radius:6px;border-left:4px solid ' + C.accent + ';">' + srcLinkProcName(pn) + '🎓 ' + esc(pn) + '</div>';
                    by[pn].forEach(e => { html += '<div style="font-size:15px;color:' + C.dark + ';padding:3px 0 3px 10px;">' + srcLink(e, e.title) + nameLink(e.name, e.link) + ' — <span style="color:' + C.red + ';font-weight:700;">' + e.total.toFixed(2) + 'h</span> <span style="color:' + C.grey + ';font-size:13px;">(' + esc(e.title) + (e.manager ? ' · ' + esc(e.manager) : '') + ')</span></div>'; });
                });
        ...
        function buildLogadoErradoHTML(fr) {
            ...
            if (list.length) {
                ...
                Object.keys(by).sort((a, b) => a.localeCompare(b)).forEach(mgr => {
                    ...
                    by[mgr].forEach(e => { html += '<div style="font-size:15px;color:' + C.dark + ';padding:3px 0 3px 10px;">' + srcLink(e, e.title) + nameLink(e.name, e.link) + ' — <span style="color:' + C.red + ';font-weight:700;">' + (e.total || 0).toFixed(2) + 'h</span> <span style="color:' + C.grey + ';font-size:13px;">(' + esc(e.title) + ')</span></div>'; });
                });
        ...
        function showDashboard(r) {
            ...
            // Clique no 🔗 dentro do card abre o FCLM; não deve abrir o modal também.
            content.addEventListener('click', ev => { if (ev.target.closest('a')) return; const card = ev.target.closest('.onb-train-card'); if (!card) return; showPeopleModal(viewR.trainings[+card.dataset.idx]); });
            ...
            // Abre o relatório da ABA ativa com a janela do turno aplicada.
            // Antes usava REPORT_LINK (fixo, sem janela) e caía numa página
            // diferente da que gerou os números na tela.
            const fclmUrlNow = () => { const p = PROCESSES.find(x => x.key === currentProc); return reportUrl(p && p.processId ? p.processId : ONB_PROCESS); };
            const btnFclm = document.createElement('button'); btnFclm.innerHTML = '🔗 Abrir no FCLM'; btnFclm.title = 'Abre o relatório da aba atual, já filtrado pelo turno'; btnFclm.style.cssText = 'background:#fff;color:' + C.dark + ';border:1px solid #CDD4DA;padding:9px 18px;border-radius:8px;cursor:pointer;font-weight:700;font-size:13px;'; btnFclm.onclick = () => { const u = fclmUrlNow(); try { window.open(u, '_blank', 'noopener'); } catch (e) { location.href = u; } };
        ...
        function injectOverlay(r) {
            ...
            headLeft.innerHTML = '<div style="font-size:14px;font-weight:700;">' + (onbFlowNow ? ((dia ? '☀️' : '🌙') + ' Alertas de Onboarding <span style="font-size:11px;font-weight:600;opacity:.85;">(' + (dia ? 'Dia' : 'Noite') + ')</span>') : ((dia ? '☀️' : '🌙') + ' Learning Hours <span style="font-size:11px;font-weight:600;opacity:.85;">(' + (dia ? 'Dia' : 'Noite') + ')</span>')) + '</div>';
            ...
            const fnHeader = (fn) => '<div style="margin:12px 0 4px;font-size:13px;font-weight:800;color:#fff;background:' + C.dark + ';padding:6px 10px;border-radius:6px;border-left:4px solid ' + C.accent + ';">' + srcLink(fn, fn) + '🎓 ' + esc(fn) + '</div>';
            ...
            // "Precisa logar" agrupado pelo CALM CODE que falta (não mais por gestor).
            ...
            hintEl.style.cssText = 'flex-shrink:0;padding:6px 14px;background:#fff;border-bottom:1px solid ' + C.border + ';font-size:10.5px;font-weight:600;color:' + C.grey + ';line-height:1.4;';
            hintEl.innerHTML = 'Janela do turno atual (' + (dia ? 'Dia 05:30→18:00' : 'Noite 18:00→05:30') + '). 🔗 Clique no link ao lado de cada item para abrir no FCLM o relatório desses dados.';
    ...
    function init() {
        ...
        window.addEventListener('keydown', function (e) {
            // Atalho: Ctrl+Alt+H → oculta/mostra o círculo (não afeta os alertas)
            if (e.ctrlKey && e.altKey && !e.shiftKey && (e.key === 'h' || e.key === 'H')) {
                e.preventDefault(); e.stopPropagation(); toggleFabHidden(); return;
            }
